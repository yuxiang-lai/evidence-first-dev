...
This reference is the operating procedure behind the published `SKILL.md`. Load only the
...
- command evidence: `evidence-first-dev/command-evidence-v3`
- manual evidence: `evidence-first-dev/manual-evidence-v1`

Read [schemas.md](schemas.md) for the upgrade rules and the exact v3 redaction
contract.

## Runtime independence

The Markdown ledger is the portable core and does not require Node.js, Python,
or the target project's language runtime. Use Portable Markdown Mode when
Node.js is unavailable: read and update `docs/WORKFLOW.md` by hand, run the
real project commands normally, and record fixed-format manual evidence with
`templates/EVIDENCE.md`.

Project memory is intentionally bounded. Read [retention.md](retention.md) when
`docs/CONTEXT.md` or a portable `docs/WORKFLOW.md` grows past its limits; do not
copy completed change history into the recovery entry.

Node.js 18+ is an optional Assisted mode. Its standard-library scripts automate
ledger initialization, recovery-index synchronization, structural validation,
and bounded machine evidence capture. They are adapters around the protocol,
not part of the target project's build or runtime. Read
[portable-mode.md](portable-mode.md) for the exact procedure.
...
Portable evidence is an additive, explicit extension: an existing ledger with
no `Evidence mode` keeps its machine-evidence meaning, while a ledger that
declares `Evidence mode: portable` opts into the independently versioned
`manual-evidence-v1` format. Do not silently change an existing ledger from
machine to portable mode.
...
1. Read `docs/WORKFLOW.md`. This is the cross-session recovery entry. If
   Node.js is available, run `node <skill>/scripts/index.mjs resume
   <project-root>` first to refresh it; otherwise follow
   [portable-mode.md](portable-mode.md).
...
state, or acceptance status, run this when Node.js is available:
...
refresh it. Without Node.js, manually update the status block and verify that
every active or blocked change has an ID and a `PROGRESS.md` link. A stale
index is a memory synchronization failure, not a reason to guess from the
conversation.
...
In Portable mode, use the same shape with
`[manual evidence](evidence/AC-01-check.md)`.

Keep one AC per line. The summary `Acceptance: PASS` is required as well, but
never replaces the individual lines.

The `Command` in each `REVIEW.md` AC line must match that AC's PRD `Verify`
command after normalizing whitespace. Important checks use the explicit shape
`IC-01: AC-01 or T01 | Command: <exact command> | evidence required`; their
successful evidence must come from that exact command.

The corresponding `PROGRESS.md` row is generated by `run-evidence.mjs` in
Assisted mode and has this shape:
...
`spawn-failure`, or `signal`. In Portable mode, create a manual evidence file
from `templates/EVIDENCE.md`, then add the equivalent row with a
`[manual evidence]` link. Do not claim machine capture, invent an evidence
filename, or replace an individual AC line with a summary.
...
runner when Node.js is available:
...
Without Node.js, run the important check normally and write a separate
`manual-observed` file using `templates/EVIDENCE.md`. Portable mode accepts
that file for a successful check, but its limitations must remain visible.
...
On resume, start at the project entry. With Node.js:
...
Without Node.js, follow the same order manually from `docs/WORKFLOW.md` and
continue at the first incomplete task. The protocol is identical; only index,
validation, and evidence capture are manual.
