...
TDD（测试驱动开发）不是“先写很多测试”，而是先用一个最小测试定义期望行为，
再让实现沿着可验证的反馈循环前进：

1. **Red**：先写一个因目标行为尚未实现而失败的测试，并确认失败原因正确。
2. **Green**：只写让当前测试通过所需的最小代码，不顺手扩展范围。
3. **Refactor**：在测试持续通过的保护下消除重复、改善命名和结构。
4. **Verify**：先跑聚焦测试，再运行与改动风险相称的更大范围检查。

测试描述的是用户或系统可观察的结果，而不是把内部实现方式写死。处理 bug 时，
原始复现就是 Red 阶段：先稳定复现，提出可证伪的根因假设，修复后再用覆盖
原始失败路径的回归测试锁定行为。

为了避免形式主义：纯文档、不可自动化的视觉判断、一次性配置或真正微小的
低风险修改，不必强行引入测试框架；使用最小可执行验收检查或诚实的人工观察
即可。

**解决的问题**：避免靠猜测修改代码，也避免某次碰巧通过后就误以为问题已经
解决；同时为 AI 提供短反馈循环，使每一步都能证明“为什么改、改了什么、是否有效”。
