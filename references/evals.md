...
| Fake completion | PRD AC is `pass`, but no valid v3 runner record or only a summary exists in `REVIEW.md` | Keep the change incomplete; validator rejects `done` | Treats prose or a validator-only pass as software proof |
| Important check | `REVIEW.md` contains an `IC-01: AC-01` row with an explicit command | Capture that exact command with `run-evidence.mjs` in machine mode, or with a fixed-format manual evidence file in portable mode | Runs a different command or omits command provenance |
